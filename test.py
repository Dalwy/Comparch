            ...
            while pre_alu_buff[0] and pre_alu_buff[1] == -1:
                ...
                for i in range(len(team12_Disassembler.instructions)):
                    ...
                    if post_alu_buff[0] == -1:
                        ...
                        #SUB
                        elif self.opcodes == 1624:
                            r[self.dest] = r[self.src1] - self.src2
                            post_alu_buff[0] = pre_alu_buff[0]
                        #SUBI
                        elif 1672 <= self.opcodes <= 1673:
                            r[self.dest] = r[self.src1] - self.src2
                            post_alu_buff[0] = pre_alu_buff[0]
                        #AND
                        elif self.opcodes == 1104:
                            r[self.dest] = r[self.src1] & r[self.src2]
                            post_alu_buff[0] = pre_alu_buff[0]
                        #ORR
                        elif self.opcodes == 1360:
                            r[self.dest] = r[self.src1] | r[self.src2]
                            post_alu_buff[0] = pre_alu_buff[0]
                        #EOR
                        elif self.opcodes == 1872:
                            r[self.dest] = r[self.src1] ^ r[self.src2]
                            post_alu_buff[0] = pre_alu_buff[0]
                        #MOVZ
                        elif 1684 <= self.opcodes <= 1687:
                            p_num = pow(2,self.src1)
                            r[self.dest] = p_num * self.src2
                            post_alu_buff[0] = pre_alu_buff[0]
                        #MOVK
                        elif 1940 <= self.opcodes <= 1943:
                            p_num = pow(2, self.src1)
                            r[self.dest] = p_num * self.src2
                            post_alu_buff[0] = pre_alu_buff[0]
                        #STUR
                        elif self.opcodes == 1984:
                            post_alu_buff[0] = pre_alu_buff[0]
                        #LDUR
                        elif self.opcodes == 1986:
                            post_alu_buff[0] = pre_alu_buff[0]
                        #CBZ
                        elif 1440 <= self.opcodes <= 1447:
                            if r[self.dest] == 0:
                                offset = self.src1
                                r[self.dest] += offset - 1
                            post_alu_buff[0] = pre_alu_buff[0]
                        #CBNZ
                        elif 1448 <= self.opcodes <= 1455:
                            if r[self.dest] != 0:
                                offset = self.src1
                                r[self.dest] += offset - 1
                            post_alu_buff[0] = pre_alu_buff[0]
                        #ASR
                        elif self.opcodes == 1692:
                            r[self.dest] = self.src1 / self.src2
                            post_alu_buff[0] = pre_alu_buff[0]
                        #LSL
                        elif self.opcodes == 1691:
                            p_num = pow(2, self.src2)
                            r[self.dest[post_alu_buff[0]]] = p_num * r[self.src1]
                            post_alu_buff[0] = pre_alu_buff[0]
                        #LSR
                        elif self.opcodes == 1690:
                            p_num = pow(2, self.src2)
                            r[self.dest[post_alu_buff[0]]] = r[self.src1] / p_num
                            post_alu_buff[0] = pre_alu_buff[0]
                        #NOP
